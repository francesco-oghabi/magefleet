...
wardenNetworkName=$(cat ${MAGEFLEET_DIR}/docker/docker-compose.yml | grep -A3 'networks:' | tail -n1 | sed -e 's/[[:blank:]]*name:[[:blank:]]*//g')
...
OLDIFS="$IFS";
IFS=$'\n'
projectNetworkList=( $(docker network ls --format '{{.Name}}' -q --filter "label=dev.magefleet.environment.name") )
IFS="$OLDIFS"
...
for projectNetwork in "${projectNetworkList[@]}"; do
    ...
    projectName=$(cat "${projectDir}/.env" | grep '^MAGEFLEET_ENV_NAME=' | sed -e 's/MAGEFLEET_ENV_NAME=[[:space:]]*//g')
    projectType=$(cat "${projectDir}/.env" | grep '^MAGEFLEET_ENV_TYPE=' | sed -e 's/MAGEFLEET_ENV_TYPE=[[:space:]]*//g')
