...
if [[ ! -d "${MAGEFLEET_SSL_DIR}/rootca" ]]; then
    mkdir -p "${MAGEFLEET_SSL_DIR}/rootca"/{certs,crl,newcerts,private}

    touch "${MAGEFLEET_SSL_DIR}/rootca/index.txt"
    echo 1000 > "${MAGEFLEET_SSL_DIR}/rootca/serial"
fi

# create CA root certificate if none present
if [[ ! -f "${MAGEFLEET_SSL_DIR}/rootca/private/ca.key.pem" ]]; then
  echo "==> Generating private key for local root certificate"
  openssl genrsa -out "${MAGEFLEET_SSL_DIR}/rootca/private/ca.key.pem" 2048
fi
...
if [[ ! -f "${MAGEFLEET_SSL_DIR}/rootca/certs/ca.cert.pem" ]]; then
  ...
  openssl req -new -x509 -days 7300 -sha256 -extensions v3_ca \
    -config "${MAGEFLEET_DIR}/config/openssl/rootca.conf"        \
    -key "${MAGEFLEET_SSL_DIR}/rootca/private/ca.key.pem"        \
    -out "${MAGEFLEET_SSL_DIR}/rootca/certs/ca.cert.pem"         \
...
if [[ "$OSTYPE" =~ ^linux ]] \
  ...
  && [[ ! -f /etc/pki/ca-trust/source/anchors/magefleet-proxy-local-ca.cert.pem ]] \
  ## Fedora/CentOS
then
  echo "==> Trusting root certificate (requires sudo privileges)"  
  sudo cp "${MAGEFLEET_SSL_DIR}/rootca/certs/ca.cert.pem" /etc/pki/ca-trust/source/anchors/magefleet-proxy-local-ca.cert.pem
  sudo update-ca-trust
elif [[ "$OSTYPE" =~ ^linux ]] \
  && [[ -d /usr/local/share/ca-certificates ]] \
  && [[ ! -f /usr/local/share/ca-certificates/magefleet-proxy-local-ca.crt ]] \
  ## Ubuntu/Debian
then
  echo "==> Trusting root certificate (requires sudo privileges)"
  sudo cp "${MAGEFLEET_SSL_DIR}/rootca/certs/ca.cert.pem" /usr/local/share/ca-certificates/magefleet-proxy-local-ca.crt
...
then
  ...
  sudo security add-trusted-cert -d -r trustRoot \
    -k /Library/Keychains/System.keychain "${MAGEFLEET_SSL_DIR}/rootca/certs/ca.cert.pem"
fi
...
if [[ "$OSTYPE" == "darwin"* ]]; then
  ...
  warning "Manual configuration required for Automatic DNS resolution: https://docs.magefleet.dev/configuration/dns-resolver.html"
fi

## generate rsa keypair for authenticating to warden sshd service
if [[ ! -f "${MAGEFLEET_HOME_DIR}/tunnel/ssh_key" ]]; then
  echo "==> Generating rsa key pair for tunnel into sshd service"
  mkdir -p "${MAGEFLEET_HOME_DIR}/tunnel"
  ssh-keygen -b 2048 -t rsa -f "${MAGEFLEET_HOME_DIR}/tunnel/ssh_key" -N "" -C "user@tunnel.magefleet.test"
fi
...
if [[ "$OSTYPE" =~ ^linux ]] && [[ "$(stat -c '%U' "${MAGEFLEET_HOME_DIR}/tunnel/ssh_key.pub")" != "root" ]]; then
  sudo chown root:root "${MAGEFLEET_HOME_DIR}/tunnel/ssh_key.pub"
fi
...
if [[ ! -f "${MAGEFLEET_HOME_DIR}/.env" ]]; then
	cat >> "${MAGEFLEET_HOME_DIR}/.env" <<-EOT
		# Set to "1" to enable global Portainer service
		MAGEFLEET_PORTAINER_ENABLE=0
		# Set to "0" to disable DNSMasq
		MAGEFLEET_DNSMASQ_ENABLE=1
		# Set to "0" to disable phpMyAdmin
		MAGEFLEET_PHPMYADMIN_ENABLE=1
		# Set to "1" to enable Mutagen
		MAGEFLEET_MUTAGEN_ENABLE=1
	EOT
fi
