...
## disable sync command when MAGEFLEET_MUTAGEN_ENABLE is set to 0
...
if [[ ${MAGEFLEET_ENV_SUBT} != "darwin" ]]; then
  fatal "Mutagen sync sessions are not used on \"${MAGEFLEET_ENV_SUBT}\" host environments."
fi
...
if [[ ! -f "${MUTAGEN_SYNC_FILE}" ]]; then
  fatal "Mutagen configuration does not exist for environment type \"${MAGEFLEET_ENV_TYPE}\""
fi
...
case "${MAGEFLEET_PARAMS[0]}" in
    start)
        ...
        mutagen sync terminate --label-selector "warden-sync=${MAGEFLEET_ENV_NAME}"

        ## create sync session based on environment type configuration
        mutagen sync create -c "${MUTAGEN_SYNC_FILE}" \
            --label "warden-sync=${MAGEFLEET_ENV_NAME}" --ignore "${MAGEFLEET_SYNC_IGNORE:-}" \
            "${MAGEFLEET_ENV_PATH}${MAGEFLEET_WEB_ROOT:-}" "docker://$($MAGEFLEET_BIN env ps -q php-fpm)/var/www/html"

        ## wait for sync session to complete initial sync before exiting
        echo "Waiting for initial synchronization to complete"
        while ! mutagen sync list --label-selector "warden-sync=${MAGEFLEET_ENV_NAME}" \
            | grep -i 'watching for changes'>/dev/null;
                do
                    if mutagen sync list --label-selector "warden-sync=${MAGEFLEET_ENV_NAME}" \
                        | grep -i 'Last error' > /dev/null; then
                        MUTAGEN_ERROR=$(mutagen sync list --label-selector "warden-sync=${MAGEFLEET_ENV_NAME}" \
    ...
    list|flush|monitor|pause|reset|resume)
        mutagen sync "${MAGEFLEET_PARAMS[@]}" "${@}" --label-selector "warden-sync=${MAGEFLEET_ENV_NAME}"
        ;;
    *)
