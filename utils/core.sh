...
function regeneratePMAConfig() {
  if [[ -f "${MAGEFLEET_HOME_DIR}/.env" ]]; then
    # Recheck PMA since old versions of .env may not have MAGEFLEET_PHPMYADMIN_ENABLE setting
  ...
  if [[ "${MAGEFLEET_PHPMYADMIN_ENABLE}" == 1 ]]; then
    ...
    pma_config_file="${MAGEFLEET_HOME_DIR}/etc/phpmyadmin/config.user.inc.php"
